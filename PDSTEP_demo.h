...
// Network parameters:
...
#ifdef EXPORT
	tuple < vector<vector<float > >, vector<vector<int > > > stepSNN(vector<float > a, vector<float > b, vector<float > c, vector<float > d, vector<float > v, vector<float > u, vector<vector<float > > w, vector<double > sensor_val, int num_output, int neur_sim_step, int simStep);
	vector<vector<int > > firings; // matrix that holds binary spike data
#else // if no EXPORT:
	vector<vector<float > > stepSNN(vector<float > a, vector<float > b, vector<float > c, vector<float > d, vector<float > v, vector<float > u, vector<vector<float > > w, vector<double > sensor_val, int num_output, int neur_sim_step);
#endif // EXPORT

	// function that reads wts from a file
	//void initParams(const std::string& inputFileName); 
	int initParams(const std::string& inputCueTime);
	...
	virtual void stepPhysics(int cue_time);
